...
namespace FashionShop.Presentation
{
    public sealed partial class UcInventory : UserControl, IReloadable
    {
        ...
        public UcInventory()
        {
            ...
            // Khởi tạo service
            _invService = new InventoryService(new FashionShopDb());
        ...
        private void Dgv_CellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            ...
            if (e.ColumnIndex == colStatus)
            {
                ...
                Color c = C_TEXT;
                if (status == "TỒN THẤP") c = C_RED;
                else if (status == "SẴN SÀNG") c = C_GREEN;
                else if (status == "ĐANG NHẬP") c = C_BLUE;
                else if (status == "TỒN NHIỀU") c = C_ORANGE;
        ...
        private void LoadDataFromDb()
        {
            try
            {
                var data = _invService.GetAll();

                _all = data.Select((x, idx) => new InvRow(
                    idx + 1,
                    x.Sku,
                    x.FullProductName,
                    x.Location,
                ...
                // Cập nhật KPI cards
                vTotalProducts.Text = _invService.GetTotalProducts().ToString("#,0");
                vStockValue.Text = _invService.GetTotalStockValue().ToString("#,0") + "đ";
                vLowAlert.Text = _invService.GetLowAlertCount() + " SKU";
                vInbound.Text = _invService.GetInboundOrders() + " đơn";
        ...
        private static string BuildStatus(int stock)
        {
            if (stock == 0) return "ĐANG NHẬP";
            if (stock <= 10) return "TỒN THẤP";
            if (stock >= 160) return "TỒN NHIỀU";
            return "SẴN SÀNG";
        }
        ...
        private sealed class InvRow
        {
            ...
            public InvRow(int stt, string sku, string name, string location, DateTime? date, int? quantity)
            {
                Stt = stt;
                Sku = sku;
                Name = name;
                ...
                Stock = quantity ?? 0; // ← Nếu quantity NULL → gán 0
                Status = BuildStatus(Stock); // ← Dùng Stock sau khi đã gán
            }
        }
