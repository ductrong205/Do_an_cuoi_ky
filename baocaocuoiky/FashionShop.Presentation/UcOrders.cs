...
namespace FashionShop.Presentation
{
    public partial class UcOrders : UserControl, IReloadable
    {
        ...
        public UcOrders()
        {
            ...
            _orderService = new OrderService(new FashionShopDb());
        ...
        private void Wire()
        {
            btnNew.Click += (s, e) =>
            {
                var frm = new FrmOrderCreate();
                if (frm.ShowDialog() == DialogResult.OK && frm.CreatedOrder != null)
                {
                    var newOrder = frm.CreatedOrder;
                    ...
                    // Lấy danh sách sản phẩm (từ OrderItems)
                    string productSummary;
                    using (var db = new FashionShopDb())
                    {
                        var products = (from oi in db.OrderItems
                                        join p in db.Products on oi.ProductId equals p.ProductId
                                        where oi.OrderId == newOrder.OrderId
                                        select p.ProductName)
                                       .Take(3)
                                       .ToList();
                        productSummary = products.Any() ? string.Join(", ", products) : "Không có sản phẩm";
                    }

                    var newRow = new OrderRow(
                        newOrder.OrderCode,
                        newOrder.OrderDate.ToString("HH:mm, dd/MM/yyyy"),
                        customerName,
                        productSummary,
                        newOrder.TotalAmount,
                        MapStatus(newOrder.Status)
                    );

                    // Thêm vào đầu danh sách
                    _all.Insert(0, newRow);

                    // Cập nhật lại filter & paging (có thể đang ở tab "Tất cả" hoặc "Chờ xử lý")
                    ApplyFilterAndPaging();
            ...
            btnAdv.Click += (s, e) => {
                var frm = new FrmOrderAdvancedFilter();
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    ApplyAdvancedFilter(frm.FromDate, frm.ToDate, frm.CustomerName, frm.MinTotal, frm.MaxTotal, frm.Status);
                }
            };

            btnExport.Click += (s, e) => ExportCsv();

            var btnAutoUpdate = MakeButton("🔄 Cập nhật trạng thái", C_PANEL2, C_TEXT);
            btnAutoUpdate.Margin = new Padding(0, 8, 10, 0);
            flRight.Controls.Add(btnAutoUpdate); // hoặc thêm vào flLeft nếu muốn

            btnAutoUpdate.Click += (s, e) =>
            {
                try
                {
                    using (var db = new FashionShopDb())
                    {
                        // Lấy tất cả đơn hàng ra bộ nhớ (AsEnumerable)
                        var allOrders = db.Orders.AsEnumerable().ToList();

                        // 1. Chuyển "Chờ xử lý" → "Vận chuyển" sau 1 phút
                        var pendingOrders = allOrders
                            .Where(o => o.Status == 0 && o.OrderDate <= DateTime.Now.AddMinutes(-1))
                            .ToList();
                        ...
                        // 2. Chuyển "Vận chuyển" → "Hoàn thành" sau 3 phút
                        var shippingOrders = allOrders
                            .Where(o => o.Status == 1 && o.UpdatedAt <= DateTime.Now.AddMinutes(-3))
                            .ToList();
                        ...
                        // 👇 CẬP NHẬT LẠI UcOrders
                        var ucOrders = Application.OpenForms.OfType<UcOrders>().FirstOrDefault();
                        if (ucOrders != null)
                        {
                            var reloadable = ucOrders as IReloadable;
                            if (reloadable != null)
                            {
                                reloadable.ReloadData();
                            }
                        ...
                        MessageBox.Show($"Đã cập nhật {pendingOrders.Count + shippingOrders.Count} đơn hàng!", "Thành công", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                catch (Exception ex)
                {
        ...
        private void LoadDataFromDb()
        {
            try
            {
                ...
                Console.WriteLine($"[DEBUG] Số đơn hàng lấy được: {orders.Count}");

                if (orders == null || orders.Count == 0) // ← Thêm dòng này
                    orders = new List<Order>();

                _all = orders.Select(o => new OrderRow(
                    o.OrderCode,
                    o.OrderDate.ToString("HH:mm, dd/MM/yyyy"),
                    o.Customer?.FullName ?? "Khách lẻ",
                    GetProductSummary(o.OrderId),
                    o.TotalAmount,
                    MapStatus(o.Status)
                )).ToList();
            }
        ...
        {
            Console.WriteLine($"[DEBUG] Status: {status}"); // ← Xem giá trị thực tế

            switch (status)
            {
                case 0: return OrderStatus.Pending;
                case 1: return OrderStatus.Shipping;
                case 2: return OrderStatus.Completed;
                case 3: return OrderStatus.Cancelled;
                default:
                    Console.WriteLine($"[ERROR] Status không hợp lệ: {status}");
                    return OrderStatus.Pending;
            }
        }

        private string GetProductSummary(int orderId)
        {
            try
            {
                using (var db = new FashionShopDb())
        ...
        // ===================== LỌC NÂNG CAO (MỚI THÊM - TƯƠNG THÍCH C# 7.3) =====================
        private void ApplyAdvancedFilter(DateTime? fromDate, DateTime? toDate, string customerName, decimal? minTotal, decimal? maxTotal, string status)
        {
            try
            {
                // Lọc theo trạng thái (chuyển chuỗi thành enum)
                OrderStatus? statusFilter = null;
                if (!string.IsNullOrEmpty(status))
                {
                    if (status == "Chờ xử lý") statusFilter = OrderStatus.Pending;
                    else if (status == "Đang giao") statusFilter = OrderStatus.Shipping;
                    else if (status == "Hoàn thành") statusFilter = OrderStatus.Completed;
                    else if (status == "Hủy") statusFilter = OrderStatus.Cancelled;
                }
                ...
                if (fromDate.HasValue)
                    filtered = filtered.Where(o => DateTime.Parse(o.TimeText) >= fromDate.Value);
                if (toDate.HasValue)
                    filtered = filtered.Where(o => DateTime.Parse(o.TimeText) <= toDate.Value);

                // Lọc theo khách hàng
                if (!string.IsNullOrEmpty(customerName) && customerName != "(Tất cả)")
                {
                    filtered = filtered.Where(o => o.Customer.IndexOf(customerName, StringComparison.OrdinalIgnoreCase) >= 0);
                }

                // Lọc theo tổng tiền — sử dụng tham số đã truyền vào
                if (minTotal.HasValue)
                    filtered = filtered.Where(o => o.Total >= minTotal.Value);

                if (maxTotal.HasValue)
                    filtered = filtered.Where(o => o.Total <= maxTotal.Value);
                ...
                _page = 1;
                _totalPages = Math.Max(1, (int)Math.Ceiling(_view.Count / (double)_pageSize));
                ApplyFilterAndPaging();
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Lỗi khi lọc nâng cao: {ex.Message}", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
