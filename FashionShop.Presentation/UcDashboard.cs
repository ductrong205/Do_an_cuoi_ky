...
namespace FashionShop.Presentation
{
    public partial class UcDashboard : UserControl, IReloadable
    {
        ...
        private void Wire()
        {
            btnRange7d.Click += (s, e) => MessageBox.Show("Demo: đổi range (7 ngày qua).");
            lnkTopDetail.Click += (s, e) => MessageBox.Show("Demo: mở danh sách sản phẩm bán chạy.");
            btnFilter.Click += (s, e) => MessageBox.Show("Demo: lọc dữ liệu đơn hàng.");
        ...
        private void LoadDemo()
        {
            ...
            int[] vals = { 4, 7, 5, 9, 6, 11, 8 };

            for (int i = 0; i < days.Length; i++)
            {
                int idx = s.Points.AddXY(days[i], vals[i]);
                var dp = s.Points[idx];

                // giống ảnh: đa số xanh đậm, T7 tím
                dp.Color = (days[i] == "T7") ? C_ACCENT : C_BLUEBAR;
            ...
            // top products
            lvTop.Items.Clear();
            AddTop("Áo thun Basic", "Trắng - Size L", "120 cái", 0);
            AddTop("Jeans Slim Fit", "Xanh - 32", "98 cái", 1);
            AddTop("Khoác Bomber Da", "Nâu - M", "45 cái", 2);
            AddTop("Hoodie Streetwear", "Đen - Oversize", "32 cái", 3);
            FixTopCols();

            // orders grid
            _orders.Clear();
            _orders.Add(new OrderRow("ORD-7532", "Lê Minh", "24/10/2023", 1250000, "Hoàn thành"));
            _orders.Add(new OrderRow("ORD-7531", "Trần Thị B", "24/10/2023", 850000, "Vận chuyển"));
            _orders.Add(new OrderRow("ORD-7530", "Nguyễn Khoa", "23/10/2023", 2100000, "Chờ xử lý"));
            _orders.Add(new OrderRow("ORD-7529", "Hoàng Nam", "23/10/2023", 450000, "Đã hủy"));

            dgv.DataSource = null;
            dgv.DataSource = _orders;

            lbBottomInfo.Text = "Hiển thị 4 trên 150 đơn hàng";
        }
