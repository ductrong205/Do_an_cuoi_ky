...
namespace FashionShop.Presentation
{
    public partial class UcReports : UserControl, IReloadable
    {
        ...
        private void BuildDonut(Panel host)
        {
            ...
            // keep centered when resize
            host.Resize += (s1, e1) =>
            {
                pnCenter.Left = (host.ClientSize.Width - pnCenter.Width) / 2;
                pnCenter.Top = 74; // chỉnh gần donut
            };
        }
        ...
        private void Dgv_CellPainting_StatusPill(object sender, DataGridViewCellPaintingEventArgs e)
        {
            ...
            if (e.ColumnIndex == 5)
            {
                ...
                if (text.IndexOf("In Stock", StringComparison.OrdinalIgnoreCase) >= 0)
                {
                    border = Color.FromArgb(40, 170, 90);
                    fore = Color.FromArgb(60, 200, 120);
                }
                else if (text.IndexOf("Low", StringComparison.OrdinalIgnoreCase) >= 0)
                {
        ...
        private void Wire()
        {
            ...
            btnPrint.Click += (s, e) =>
            {
                MessageBox.Show("In (demo). Nếu bạn muốn in thật: mình sẽ tạo PrintDocument cho bạn.");
            };

            btnExport.Click += (s, e) => ExportCsv();

            btnRefresh.Click += (s, e) => { LoadDemo(GetSelectedRange()); BindAll(); };
            btnFilter.Click += (s, e) => MessageBox.Show("Lọc (demo). Bạn muốn lọc theo gì? SKU/Trạng thái/Khoảng thời gian?");
            lnkFull.Click += (s, e) => MessageBox.Show("Xem toàn bộ dữ liệu (demo).");
        }
        ...
        // ===================== DATA =====================
        private void LoadDemo(string range)
        {
            lbH2.Text = "Khoảng thời gian: " + range + " | Cập nhật lúc: " + DateTime.Now.ToString("HH:mm");

            // Cards theo ảnh
            vRevenue.Text = "1.2 tỷ đ";
            vProfit.Text = "350 tr đ";
            vOrders.Text = "1,240";
            vAov.Text = "960k đ";
            ...
            dRevenue.ForeColor = C_OK;
            dProfit.ForeColor = C_OK;
            dOrders.ForeColor = C_BAD;
            dAov.ForeColor = C_OK;

            // Chart cột theo tuần trong ảnh (giữ nguyên data)
            _bars = new List<BarPoint>
            {
                new BarPoint("Mon", 52, 85),
                new BarPoint("Tue", 63, 100),
                new BarPoint("Wed", 40, 70),
                new BarPoint("Thu", 64, 100),
                new BarPoint("Fri", 58, 95),
                new BarPoint("Sat", 75, 100),
                new BarPoint("Sun", 57, 100),
            };

            // Donut (đổi tên category sang Việt)
            _cats = new List<CategoryPoint>
            {
                new CategoryPoint("Thời trang nam", 45, Color.FromArgb(0,120,215)),
                new CategoryPoint("Thời trang nữ", 25, Color.FromArgb(255, 80, 160)),
                new CategoryPoint("Phụ kiện", 20, Color.FromArgb(0, 160, 200)),
                new CategoryPoint("Khác", 10, Color.FromArgb(100, 100, 110)),
            };

            // Grid products (giữ nguyên)
            _products = new List<ProductRow>
            {
                ProductRow.Make(1, "Áo Thun Basic White", "SKU-9012", 432, "In Stock", 125000000),
                ProductRow.Make(2, "Quần Short Denim", "SKU-1029", 321, "Low Stock", 98500000),
                ProductRow.Make(3, "Giày Sneaker Sport Red", "SKU-5521", 210, "In Stock", 250000000),
                ProductRow.Make(4, "Đầm Hoa Mùa Hè", "SKU-3320", 180, "In Stock", 90000000),
            };
        }
        ...
        private void BindDonut()
        {
            ...
            for (int i = 0; i < _cats.Count; i++)
            {
                ...
                var colorBox = new Panel { Width = 10, Height = 10, BackColor = c.Color, Margin = new Padding(0, 6, 0, 0) };
    ...
    // Nếu project bạn đã có IReloadable rồi thì xoá đoạn này.
}
